...
def plot_sim(u, Lx, Ly, xr, yr, wall_mask, scale, frequency):
    ...
    filename = f"gridbest_router_freq{frequency}_x{xr:.2f}_y{yr:.2f}.pdf"
...
def grid_search_router_parallel(Lx, Ly, nx, ny, scale, dx, walls, outer_walls,
    ...
    measurement_positions = [(1,5), (2,1), (9,1), (9,7)]

    tasks = []
    for xr in x_values:
        for yr in y_values:
            if not is_valid_position(xr, yr, measurement_positions, min_dist=0.5):
                continue
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            print(f"tested ({xr:.2f}, {yr:.2f} score {score:.2f} linear sum")
    ...
    with open(filepath, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(["xr", "yr", "score", "Living", "Kitchen", "Bathroom", "Bedroom"])
    ...
    print("Best summed score linear:", best_score)
    for name, s in zip(names, best_strengths):
        print(f"{name}: {s:.2f} dB")

    return best_pos, best_score, results

def is_valid_position(xr, yr, positions, min_dist=0.5):
    for x, y in positions:
        if np.sqrt((xr - x)**2 + (yr - y)**2) < min_dist:
            return False
    return True
...
def runhelmholtzformain():
    ...
    # x_values = np.arange(7.8, 9.3, 0.1)
    # y_values = np.arange(6.2, 7.5, 0.1)

    x_values = np.arange(8.0, 9.0, 0.05)
    y_values = np.arange(6.7, 7.4, 0.05)
