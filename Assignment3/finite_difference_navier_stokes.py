...
def animate_flow(X: np.ndarray,
    ...
    """
    Animate the evolution of the flow field.
    ...
    X, Y : np.ndarray
        Grid coordinates
    u_hist, v_hist : list[np.ndarray]
        Stored velocity fields over time
    p_hist : list[np.ndarray]
        Stored pressure fields over time
    interval : int
    ...
    def update(frame: int):
        ...
        Each frame plots a different timestep of the simulation.
        """
        ...
        # pressure contours
        contour = ax.contourf(X, Y, p, alpha=0.5, cmap=cm.viridis)

        # pressure lines
        ax.contour(X, Y, p, cmap=cm.viridis)

        # velocity streamlines
        ax.streamplot(X, Y, u, v, density=1.5, color='k')
    ...
    plt.show()
    ani.save("flow_animation.mp4", fps=30)
