...
def run_1_3():
        ...
        for k in range(max_iteration):
            ...
            for j in range(1, Ny-1):
                for i in range(Nx):
                    ...
                    c[j, i] = (1 - omega) * c[j, i] + omega * neighbour[j, i]
    ...
    def run_sor_diff_vals():
        ...
        for N in N_list:
            ...
            for omega in omega_list:
                ...
                k_list.append(k)
            iter_list.append(k_list)
            max_val = min(k_list)
            best_omegas.append(omega_list[k_list.index(max_val)])
            k_list = []
        return best_omegas, N_list, iter_list, omega_list
    ...
    c_sor, delta_list_sor, k_sor, _ = sor_iteration(c_initial.copy(), omega=1.85, obj_matrix=np.zeros((N, N)), max_iteration=500)
    ...
    error_jacobi = np.max(np.abs(c_numerical_jacobi.mean(axis=1) - c_analytical))
    print(f"Max error for Jacobi method: {error_jacobi}")
    ...
    error_gauss = np.max(np.abs(c_numerical_gauss.mean(axis=1) - c_analytical))
    print(f"Max error for Gauss-Seidel method: {error_gauss}")
    ...
    error_sor = np.max(np.abs(c_numerical_sor.mean(axis=1) - c_analytical))
    print(f"Max error for SOR method: {error_sor}")

    df = pd.DataFrame({
        "Method": ["Jacobi", "Gauss", "SOR"],
        "Errors": [error_jacobi, error_gauss, error_sor]
    })
    df.to_csv("Figures/1.3/errors.csv", index=False)
    ...
    omegas = [1.75, 1.80, 1.85, 1.90, 1.95]
    cmap = plt.get_cmap('tab20c', len(omegas) + 2)
    plt.plot(delta_list, label="Jacobi", color=cmap(0))
    plt.plot(delta_list_gauss, label="Gauss-Seidel", color=cmap(1))

    # For SOR, choose a few representative values for ω.
    for w in range(len(omegas)):
        _, delta_list_sor, _, _ = sor_iteration(c_initial.copy(), omega=omegas[w], obj_matrix=np.zeros((50,50)), max_iteration=500)
        plt.plot(delta_list_sor, label=f"SOR omega={omegas[w]}", color=cmap(w + 2))
    plt.yscale('log')
    ...
    if run_j:
        ...
        print(N_vals)
        print(omegas)
        cmap = plt.get_cmap('gist_rainbow', len(N_vals))
        for i in range(len(N_vals)):
            plt.plot(omegas, iter_list[i], color=cmap(i), label=f"N = {N_vals[i]}")
        plt.xlabel("ω")
        ...
        df = pd.DataFrame({
            "N values": N_vals,
            "Optimal omega": best_omegas  
        })
        df.to_csv("Figures/1.3/omega_test.csv", index=False)
    ...
    if run_k_omega:
        ...
        df = pd.DataFrame({
            "Num sink obj": [0, 1, 2, 3, 4, 5],
            "Optimal omega": [best0, best1, best2, best3, best4, best5]
        })
        df.to_csv("Figures/1.3/omega_test_sink_objects.csv", index=False)
